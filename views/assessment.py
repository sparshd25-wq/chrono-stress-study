...
def _generate_quasi_random_pulse() -> float:
    ...
    # Use higher internal precision so displayed animation interval (1/target)
    # and value saved from the dial are numerically identical (no scaling or
    # rounding mismatch). The visual readout remains unchanged (2 dp).
    return round(target_rate, 4)
...
def render_estimation() -> None:
    ...
    if phase == "display":
        ...
        # Use CSS animation for the reference pulse (period = 1/target_rate)
    ...
    if phase == "respond":
        ...
        target_rate = st.session_state.task_estimation_target_pulse_rate
        # No debug caption shown here either.
        ...
        if matched_rate is not None:
            ...
            # Store the exact numeric target and matched values (no unit/scale conversion)
            st.session_state.task_estimation_result = {
